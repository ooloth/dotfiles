...
2. **Check CI failures** (recursionpharma repos only):
   - If tests pass locally but fail in CI, use the `inspect-codefresh-failures` skill
...
- **Update CI/CD** - Integrate new tests into automated pipelines
  ...
  - If CI fails, use the `inspect-codefresh-failures` skill to analyze build logs
