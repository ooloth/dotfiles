...
3. **Gather information** - Error messages, stack traces, environment
   - **For recursionpharma repos with CI failures**: Use the `inspect-codefresh-failures` skill to analyze Codefresh build logs and identify environment differences
...
- For CI failures (recursionpharma): Use the `inspect-codefresh-failures` skill
